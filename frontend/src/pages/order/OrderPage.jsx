...
const OrderPage = () => {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 bg-gray-100 min-h-screen">
      ...
      {/* If no orders */}
      {orders.length === 0 && (
        <div className="text-center text-base sm:text-lg font-medium text-gray-600">
          No orders found 😕
        </div>
      )}
      ...
      <div className="space-y-6">
        {orders.map((order) => (
          <div
            key={order._id}
            className="bg-white rounded-xl p-4 sm:p-5 shadow-md hover:shadow-lg transition"
          >
            {/* Order Header */}
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 border-b pb-3">
              <h2 className="text-sm sm:text-lg font-semibold break-all">Order ID: {order._id}</h2>
                ...
                return (
                  ...
                  >
                    <img
                      ...
                      alt=""
                      className="w-full sm:w-32 md:w-40 h-40 sm:h-20 object-cover rounded-lg"
                    />
            ...
            {/* Order Footer */}
            <div className="mt-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-t pt-3">
              <div>
                <p className="text-gray-600 text-sm">Payment</p>
                <p className="text-base sm:text-lg font-bold">₹ {order.price?.totalAmount || 0}</p>
              </div>
